    ...
    _In_ ULONG rgbMask)
{
    if (m_subType == MFVideoFormat_RGB32)
    {
        ...
        // Real iPhone video if the desktop app is publishing it, otherwise
        // the generated pattern. Never nothing - see SharedFrameReader.h.
        if (!m_sharedFrames.ReadInto(pBuf, pitch, m_width, m_height))
        {
    ...
    else if(m_subType == MFVideoFormat_NV12)
    {
        ...
        // Same choice as the RGB32 path above; the conversion below is
        // unchanged, so NV12 consumers get the iPhone picture too.
        if (!m_sharedFrames.ReadInto(spBuff.get(), m_width * 4, m_width, m_height))
        {
